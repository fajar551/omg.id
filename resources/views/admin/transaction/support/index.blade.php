...
@section('content')
            ...
            <div class="col-md-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-soft-primary">
                       ...
                       <li class="breadcrumb-item active" aria-current="page">@lang('page.transaction_support')</li>
                    </ol>
                 </nav>
            </div>
        </div>
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                           <h5 class="card-title">@lang('page.transaction_support')</h5>
                        </div>
                                    ...
                                    <div class="col-sm-6 col-lg-4">
                                        <div class="card card-dark-80">
                                            <div class="card-body">
                                                ...
                                                <div>@lang('page.transaction_support_total')</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-lg-4">
                                        <div class="card card-dark-80">
                                            <div class="card-body">
                                                <div id="creatoramount">
                                                    <h3></h3>
                                                </div>
                                                <div>@lang('page.transaction_support_creator_amount')</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-lg-4">
                                        <div class="card card-dark-80">
                                            <div class="card-body">
                                                <div id="platformamount">
                                                    <h3></h3>
                                                </div>
                                                <div>@lang('page.transaction_support_platform_amount')</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                                            ...
                                                            <div class="row">
                                                                ...
                                                                <div class="col-md-4 ps-3 pe-3">
                                                                    <div class="form-group">
                                                                        <label class="form-label" for="paymenttype">@lang('page.payment_type')</label>
                                                                        <select type="text" name="payment_type" id="payment_type" class="form-select " placeholder="@lang('-')"></select>
                                                                    </div>
                                                                </div>
                                                                <div class="col-md-4 ps-3 pe-3">
    ...
    <form action="{{ route('admin.transaction.support.exportexcel') }}" method="get" id="export_excel" hidden>
        @csrf
        @method("get")
        <input type="hidden" name="start_at" id="ex_start_at">
        <input type="hidden" name="end_at" id="ex_end_at">
        <input type="hidden" name="payment_type" id="ex_payment_type">
    ...
    <script type="text/javascript">
        ...
        const dtTransactionSupport = () => {
            table = $('#table').DataTable({
                ...
                searching: false,
                ajax: {
                    url: "{{ route('admin.transaction.support.dtlist') }}",
                    type: 'GET',
                    data: function(d){
                        // console.log(d);
                ...
                columns: [
                    ...
                    {data: 'date', name: 'date', width: '10%', orderable: false},
                    {data: 'status', name: 'status', width: '10%', orderable: false},
                    {data: 'detail', name: 'detail', width: '30%', orderable: false},
                    {data: 'payment_type', name: 'payment_type', width: '5%', visible: false, defaultContent: 'N/A'},
                    {data: 'gross_amount', name: 'gross_amount', width: '5%', visible: false, defaultContent: 'N/A'},
                    {data: 'pg_fee', name: 'pg_fee', width: '5%', visible: false, defaultContent: 'N/A'},
                    {data: 'pg_amount', name: 'pg_amount', width: '5%', visible: false, defaultContent: 'N/A'},
                    {data: 'platform_fee', name: 'platform_fee', width: '5%', visible: false, defaultContent: 'N/A'},
                    {data: 'platform_amount', name: 'platform_amount', width: '5%', visible: false, defaultContent: 'N/A'},
                    {data: 'creator_amount', name: 'creator_amount', width: '5%', visible: false, defaultContent: 'N/A'},
                ],
                destroy: true
            });
        }

        const totalsupport = async () => {
            ...
            const params = {'creator_id': creator_id, 'start_at': start_at, 'end_at': end_at, 'payment_type': payment_type}
            var url = "{{ route('admin.transaction.support.totalsupport') }}";
            await axios.post(url, params)
                .then(function (response) {
                    const { data } = response;
                    const { message, status } = data;
                    console.log(data);
                    $("#total_support h3").html(data+"X");
                })
        ...
        const creatoramount = async () => {
            ...
            let end_at = $('input[name=end_at]').val();
            const params = {'creator_id': creator_id, 'start_at': start_at, 'end_at': end_at, 'payment_type': payment_type}
            var url = "{{ route('admin.transaction.support.creatoramount') }}";
            await axios.post(url, params)
                .then(function (response) {
                    const { data } = response;
                    const { message, status } = data;
                    console.log(data);
                    $("#creatoramount h3").html(data);
                })
                .catch(function (error) {
                    console.log(error);
                });
        ...
        const platformamount = async () => {
            ...
            let end_at = $('input[name=end_at]').val();
            const params = {'creator_id': creator_id, 'start_at': start_at, 'end_at': end_at, 'payment_type': payment_type}
            var url = "{{ route('admin.transaction.support.platformamount') }}";
            await axios.post(url, params)
                .then(function (response) {
                    const { data } = response;
                    const { message, status } = data;
                    console.log(data);
                    $("#platformamount h3").html(data);
                })
                .catch(function (error) {
                    console.log(error);
                });
        ...
        const paymentType = async () => {
            ...
            await axios.get(url)
                .then(function (response) {
                    ...
                    document.getElementById("payment_type").innerHTML = str;
        ...
        const exportexcel = () => {
            ...
            $('#ex_payment_type').val(payment_type);
