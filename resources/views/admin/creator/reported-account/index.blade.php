...
@section('title',  __('Manage Creator - Reported Account'))
...
@section('content')
            ...
            <div class="col-md-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-soft-primary">
                       ...
                       <li class="breadcrumb-item active" aria-current="page">@lang('page.report_account')</li>
                    </ol>
                 </nav>
            </div>
        </div>
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                           <h5 class="card-title">@lang('page.report_account')</h5>
                        </div>
    ...
    <form action="{{ route('admin.creator.reportedaccount.suspend') }}" method="POST" id="set-suspend-form" hidden >
        @csrf
        @method("POST")
        <input type="hidden" name="id" id="setsuspend-id" required/>
    </form>
    <form action="{{ route('admin.creator.reportedaccount.unsuspend') }}" method="POST" id="set-unsuspend-form" hidden >
        @csrf
        @method("POST")
    ...
    <script type="text/javascript">
        ...
        const dtReport = () => {
            table = $('#table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.creator.reportedaccount.creatorprocess') }}",
                    data: function(d){
                        // console.log(d);
                    }
                },
        ...
        const dtReportDone = () => {
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.creator.reportedaccount.getlistcreatordone') }}",
                    data: function(d){
                        // console.log(d);
                    }
                },
