                        ...
                        <div class="card-header bg-transparent border-0 p-1 d-flex flex-column gap-2">
                            <div class="d-flex justify-content-between align-items-center w-100">
                                ...
                                <div class="d-flex gap-2">
                                    <button class="btn btn-outline-secondary btn-sm" id="product-prev" type="button"><i class="ri-arrow-left-s-line"></i></button>
                                    <button class="btn btn-outline-secondary btn-sm" id="product-next" type="button"><i class="ri-arrow-right-s-line"></i></button>
                                </div>
                            </div>
                            {{-- Tabs kategori produk --}}
                            <ul class="nav nav-pills mt-2" id="product-category-tabs">
                                <li class="nav-item"><a class="nav-link{{ request('type') ? '' : ' active' }}" href="?type=">Semua</a></li>
                                <li class="nav-item"><a class="nav-link{{ request('type') == 'buku' ? ' active' : '' }}" href="?type=buku">Buku</a></li>
                                <li class="nav-item"><a class="nav-link{{ request('type') == 'ebook' ? ' active' : '' }}" href="?type=ebook">Ebook</a></li>
                                <li class="nav-item"><a class="nav-link{{ request('type') == 'ecourse' ? ' active' : '' }}" href="?type=ecourse">Ecourse</a></li>
                                <li class="nav-item"><a class="nav-link{{ request('type') == 'digital' ? ' active' : '' }}" href="?type=digital">Digital</a></li>
                            </ul>
                        </div>
                            ...
                            <div class="row flex-nowrap overflow-auto" id="products-scroll" style="scroll-behavior:smooth;">
                                @if(isset($products) && count($products) > 0)
                                    @foreach($products as $product)
                                        <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 mb-3 product-card" style="min-width:250px;max-width:320px;" data-type="{{ $product->type }}">
                                            @include('products.card-product-public', ['product' => $product, 'product_id' => $product->id, 'pageName' => $pageName])
                                        </div>
                                    @endforeach
                                @else
