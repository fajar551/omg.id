...
<head>
        ...
        */
        ...
        const prefetchData = async () => {
            const params = {
                key: "{{ $key }}",
                streamKey: "{{ $streamKey }}",
                @foreach ($qParams as $paramKey => $paramValue)
                    "{{ $paramKey }}": {!! json_encode($paramValue) !!},
                @endforeach
            };

            const url = "{{ route('api.widget.overlay.show') }}?" + new URLSearchParams(params).toString();
            const options = {
                method: "GET",
                headers: {
    ...
    @if (!empty($qParams['goa_font']))
        <link href="https://fonts.googleapis.com/css?family={{ $qParams['goa_font'] }}" rel="stylesheet"
            type="text/css">
    @endif
    ...
    <style>
        ...
        .active-goal #title_tips_history,
        .active-goal p,
        .active-goal .userurl,
        .active-goal #nominal {
            display: none;
    ...
    <div id="app">
        ...
        <div class="container py-4">
            ...
            <div class="card card-goal animate__animated">
                <div
                    class="card-body text-center  {{ $qParams['goa_source'] == 'active-goal' ? 'active-goal' : 'tips-history' }}">
                    ...
                    @if ($qParams['goa_show_target_nominal'])
                        <div><span id="nominal"
                                {{ $qParams['goa_show_current_nominal'] == 1 ? 'class="d-none"' : '' }}>0</span> - <span
                                id="idrtarget" {{ $qParams['goa_show_target_nominal'] == 1 ? 'class="d-none"' : '' }}>Rp.
                                {{ $qParams['goa_custom_target'] }}</span></div>
                    @endif
                    @if ($qParams['goa_show_progress'])
                        <div class="progress">
                            <div id="progress" class="progress-bar progress-bar-striped progress-bar-animated"
                                role="progressbar" style="width: 0%" aria-valuenow="0" aria-valuemin="0"></div>
                        </div>
                    @endif
                </div>
            </div>
